...
public class LazyItemsCriticados extends LazyDataModel<Item> implements Serializable {
	...
	public LazyItemsCriticados(Items items) {
		this.items = items;
	}
	...
	public List<Item> load(int posicaoPrimeiraLinha, int maximoPorPagina,
		...
		itemsLazy = items.buscaPorPaginacaoCriticados(posicaoPrimeiraLinha,
		...
		if (getRowCount() <= 0 || (filtros != null && !filtros.isEmpty())) {
			this.setRowCount(items.countAllCriticados(filtros));
		}
